...
case $JNB_BATCH in
        LSF)
        echo -e "LSF batch system is not supported anymore. Please use SLURM instead"
        display_help 
        exit 1
        ;;
        SLURM)
...
if [[ "$JNB_GPU_MEM" ]]; then
        echo -e "Requesting $JNB_GPU_MEM memory per GPU"
        JNB_SGPU_MEM="--gres=gpumem:$JNB_GPU_MEM"
else
        JNB_SGPU_MEM=""
fi
...
case $JNB_SOFTWARE_STACK in
        old)
        JNB_MODULE_COMMAND="new gcc/4.8.2 r/3.6.0 python/3.6.1 git-lfs/2.3.0 eth_proxy"
        JNB_MODULE_COMMAND+=" ${JNB_EXTRA_MODULES[@]}"
        echo -e "Using old software stack (new gcc/4.8.2 r/3.6.0 python/3.6.1 npm/6.14.9 eth_proxy ${JNB_EXTRA_MODULES[@]})"
        ;;
        new)
        if [ "$JNB_NUM_GPU" -gt "0" ]; then
            JNB_MODULE_COMMAND="r/4.0.2 gcc/8.2.0 python_gpu/3.10.4 git-lfs/2.3.0 npm/6.14.9 eth_proxy $JNB_JULIA"
            JNB_MODULE_COMMAND+=" ${JNB_EXTRA_MODULES[@]}"
            echo -e "Using new software stack (gcc/8.2.0 python_gpu/3.10.4 r/4.0.2 npm/6.14.9 eth_proxy $JNB_JULIA ${JNB_EXTRA_MODULES[@]})"
        else
            JNB_MODULE_COMMAND="gcc/8.2.0 r/4.0.2 python/3.10.4 git-lfs/2.3.0 npm/6.14.9 eth_proxy $JNB_JULIA"
            JNB_MODULE_COMMAND+=" ${JNB_EXTRA_MODULES[@]}"
            echo -e "Using new software stack (gcc/8.2.0 python/3.10.4 r/4.0.2 npm/6.14.9 eth_proxy $JNB_JULIA ${JNB_EXTRA_MODULES[@]})"
        fi  
        ;;
        *)
...
then
ssh $JNB_SSH_OPT sbatch --ntasks=1 --cpus-per-task=$JNB_NUM_CPU --time=${JNB_RUN_TIME}:00 --mem-per-cpu=$JNB_MEM_PER_CPU_CORE $JNB_SNUM_GPU $JNB_SGPU_MEM <<ENDSBATCH
#!/bin/bash
[ -n "$JNB_MODULE_USE" ] && module use "$JNB_MODULE_USE"
module load $JNB_MODULE_COMMAND
export XDG_RUNTIME_DIR=
